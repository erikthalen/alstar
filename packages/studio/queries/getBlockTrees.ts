...
type BlocksWrapperNode<
  Name extends string,
  SubBlocks extends readonly any[],
> = DBBase & {
  readonly name: Name
...
type FieldNodeFromLiteral<F> = F extends {
  name: infer N extends string
  type: infer FT extends string
}
  ? FT extends 'blocks'
...
type FieldsFromFieldDefs<TDefs> = TDefs extends readonly any[]
  ? {
      [P in ArrayElement<TDefs> as P extends { name: infer N extends string }
        ? N
        : never]: FieldNodeFromLiteral<P>
    }
  : {}

/** A Block node (for a BlockDef literal B) */
type BlockNodeFromLiteral<B> = B extends {
  name: infer BN extends string
  type: infer BT extends string
  fields?: infer BF extends readonly any[]
}
  ? DBBase & {
      readonly name: BN
      readonly type: BT
...
type BlockNodeFromDefs<TDefs extends readonly any[]> =
  ArrayElement<TDefs> extends infer B
    ? B extends any
      ? BlockNodeFromLiteral<B>
      : never
    : never
 ...
 */
export function getBlockTrees<TStructure extends readonly any[]>(
  db: DatabaseSync,
